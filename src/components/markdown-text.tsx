...
const CodeHeader: FC<CodeHeaderProps> = ({ language, code }) => {
  ...
  const copyToClipboard = async () => {
    if (!code || copyState === "copied") return;
  ...
  const copyLabel = copyState === "copied" ? "コピーしました" : copyState === "failed" ? "コピーできませんでした" : "コードをコピー";
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              ...
              className="text-muted-foreground hover:text-foreground inline-flex size-6 items-center justify-center rounded-md transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            >
...
const defaultComponents = memoizeMarkdownComponents({
  ...
  hr: ({ className, ...props }) => <hr className={cn("aui-md-hr border-muted-foreground/20 my-4", className)} {...props} />,
  table: ({ className, ...props }) => (
    <div className="aui-md-table-wrapper my-3 w-full overflow-x-auto rounded-lg">
      <table className={cn("aui-md-table w-full min-w-max border-separate border-spacing-0", className)} {...props} />
    </div>
  ),
  th: ({ className, ...props }) => <th className={cn("aui-md-th bg-muted border-muted-foreground/20 border-s border-t px-3 py-2 text-start font-medium first:rounded-ss-lg last:rounded-se-lg last:border-e [[align=center]]:text-center [[align=right]]:text-right", className)} {...props} />,
  td: ({ className, ...props }) => <td className={cn("aui-md-td border-muted-foreground/20 border-s border-b px-3 py-2 text-start last:border-e [[align=center]]:text-center [[align=right]]:text-right", className)} {...props} />,
  tr: ({ className, ...props }) => <tr className={cn("aui-md-tr m-0 p-0 [&:last-child>td:first-child]:rounded-es-lg [&:last-child>td:last-child]:rounded-ee-lg", className)} {...props} />,
  ...
  pre: ({ className, ...props }) => <pre className={cn("aui-md-pre border-border/50 bg-muted/30 overflow-x-auto rounded-t-none rounded-b-xl border border-t-0 p-3.5 text-[13px] leading-relaxed", className)} {...props} />,
