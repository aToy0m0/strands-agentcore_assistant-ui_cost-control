...
モデル実行中のAG-UIエラーは、失敗したアシスタントメッセージとして会話内だけに表示する。同じエラーをトーストへ重複表示しない。履歴取得や割り込み回答の送信など、会話メッセージに対応しないUI操作の失敗はトーストで通知する。

CDKコンテキスト`webDebugMode=on`でブラウザデバッグを有効化できる。ON時はAG-UIクライアント標準のevent／lifecycleログと、アプリ独自の`[AgentCore Cost Control debug]`ログをConsoleへ出す。設定画面の「接続」タブでも現在値を確認できる。既定はOFFで、`webDebugMode=off`を指定した再デプロイにより無効化できる。
...
デバッグログはAuthorization、トークン、シークレット、パスワード、Base64添付をマスクします。ただし会話本文、履歴、ツール引数・結果、生のSSE本文は表示します。利用手順と注意事項は[デプロイ手順書の「ブラウザデバッグを使う」](deployment-guide.md#ブラウザデバッグを使う)を参照してください。
...
- フィードバック保存
- 添付ファイルの永続ストレージ
- 管理者画面、ユーザー管理画面（CognitoユーザーはCLIで管理）
- ブラウザからのAWSリソース設定変更
- オフライン利用
